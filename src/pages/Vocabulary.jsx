...
export default function Vocabulary() {
  ...
  const { dispatch, isWordDueForReview, isFavorite, getWordProgress } =
    useProgress();
  ...
  const filteredWords = useMemo(() => {
    ...
    if (filter === "path" && limit > 0) {
      ...
      result = result.filter((w) => isWordDueForReview(w.id));
  ...
  return (
    <div className="page-container animate-fade-in">
      <div className="page-header">
        ...
        <p>Spaced Repetition sistemi ile kelimeleri kalıcı olarak öğrenin</p>
      </div>

      {/* Session Stats */}
      <div className="vocab-session-stats">
        <span className="session-stat correct">✓ {sessionStats.correct}</span>
      ...
      {filteredWords.length === 0 ? (
              ...
              {currentWord?.family &&
                Object.keys(currentWord.family).length > 0 && (
                  <div className="flashcard-family">
                    ...
                    {Object.entries(currentWord.family).map(([type, word]) => (
        ...
        <div className="memory-game">
          ...
          {gameMatched.length === gameCards.length && gameCards.length > 0 && (
        ...
        <div className="vocab-list">
          {filteredWords.map((word) => {
            ...
            return (
              <div key={word.id} className="vocab-list-item card">
                ...
                <div className="vocab-list-meta">
                  <span
                    className="badge"
                    style={{ background: getBoxColor(word.id), color: "white" }}
                  >
                    {wp ? `Kutu ${wp.box}` : "Yeni"}
                  </span>
                  <button
                  ...
                  <button
                    className="btn btn-icon btn-sm"
                    onClick={() =>
                      dispatch({ type: "TOGGLE_FAVORITE", payload: word.id })
                    }
                  >
                    {isFavorite(word.id) ? "⭐" : "☆"}
                  </button>
